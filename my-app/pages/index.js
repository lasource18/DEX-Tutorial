...
export default function Home() {
  /* General stae variables */
  // loading is set to true when the transaction is mining and set to false when
  // loading the transaction is mined
  const [loading, setLoading] = useState(false)
  // We have two tabs in this dapp, Liquidity tab and swap tab
  // This variable keeps track of which Taab the user is on
  // If set to true this means that the user is on `liquidity` tab else he is on `swap` tab
  const [liquidityTab, setLiquidityTab] = useState(true)
  // This variable is the `0` number in form of a BigNumber
  const zero = BigNumber.from(0)
  /* Variable to keep track of amount */
  // `ethBalance` keeps track of ETH held by the user's account
  ...
  // `cdBalance` amount of `CD` tokens held by the users account
  const [cdBalance, setCDBalance] = useState(zero)
  // amount of LP tokens held by the users account
  const [lpBalance, setLPBalance] = useState(zero)
  /* Variables to keep track of liquidity to be added or removed */
  // `addEther` amount of Ether that the user wants to add to the kiquidity
  const [addEther, setAddEther] = useState(zero)
  // `addCDTokens` keeps track of the amount of CD tokens that the user wants to add to the liquidity pool
  // In case case when there is no initial liquidity and after liquidity gets added it keeps track of the
  ...
  // amount of LP tokens that the user wants to remove from the liquidity pool
  const [removeLPTokens, setRemoveLPTokens] = useState("0")
  /* Variable to keep track of swap functionality */
  // Amount that the user wants to keep
  const [swapAmount, setSwapAmount] = useState("")
  // This keeps track of the number of tokens that the user would receive after a swap
  const [tokenToBeReceivedAfterSwap, settokenToBeReceivedAfterSwap] = useState(zero)
  // Keeps track of whether `Eth` or some `Crypto Dev` token is selected
  // If `Eth` is selected it means user wants to swap `Eth` for `Crypto Dev` tokens and vice versa
  ...
  /**
   * getAmount calls various functions to retrieve amounts for ethbalance, LP tokens etc
   */
  const getAmounts = async () => {
    try {
      ...
      // get the mount of `CD` tokens present in the reserve of te `Exchange contract`
      const _reservedCD = await getReserveOfCDTokens(provider)
      // get ether reserve in the contract
  ...
  /**
   * sawpTokens: Swaps `swapAmountWei` of Eth/Crypto Dev tokens with 'tokenToBeReceivedAfterSwap` amount of Eth/Crypto Dev tokens
   */
  const _swapTokens = async () => {
    try {
      ...
      // We are using the `eq` method from BigNumber class in `ethers.js`
  ...
  const _getAmountOfTokensReceivedFromSwap = async (_swapAmount) => {
    try {
      ...
      if(!_swapAmountWEI.eq(zero)) {
        ...
        const _ethBalalnce = await getEtherBalance(provider, null, true)
  ...
  /**
   * _removeLiquidity: Removes the `removeLPTokensWei` aount of LP tokens from
   ...
   * that would be returned back to the user after he removes `removeLPTokebWei`
  ...
  // useEffects are used to react to changes in state o the website
  ...
  useEffect(() => {
    ...
    if(!walletConnected) {
      // Assign the WebModal class to the reference object by setting it's `current` value
      // This value is persisted throughout as long as this page is open
  ...
  const renderButton = () => {
    // If wallet is not connected, return a button which allows them to connecttheir wallet
    if(!walletConnected) {
      return (
    ...
    if(liquidityTab) {
      return (
        <div>
          ...
          <div>
            {/* If the reserved CD is 0, rneder the state for liquidity 0 where we ask the user 
            how much initial liquidity he wants to add else just render the state where liquidity is not zero
            and we calculate based on the `Eth` amount specified by the user how much `CD` tokens can be added */}
            {utils.parseEther(reservedCD.toString()).eq(zero) ? (
              <div>
                ...
                <input
                  ...
                  className={styles.input}
                />
                <button lassName={styles.button1} onClick={_addLiquidity}>
                  Add Liquidity
                </button>
              </div>
            ) : (
                <div>
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      // calculate the number of CD tokens
                      // that can be added given `e.target.value` amount of Eth
  ...
  return (
    <div>
      ...
      <div className={styles.main}> 
        <div>
          <h1 className={styles.title}>Welcome to the Crypto Devs Exchange!</h1>
          <div className={styles.description}>
